...
## 9. Memory & Command-Only Stabilität

### Test 9.1: Memory-Datei Auto-Erstellung
```
Starte die Extension in einem frischen Workspace ohne .vertex/memory.json und sende eine Chat-Nachricht.
```
**Erwartet:** `.vertex/memory.json` wird automatisch angelegt

### Test 9.2: Memory Persistenz und Wiederverwendung
```
Request 1: "Nutze FastAPI für das Backend und merke dir diese Entscheidung."
Request 2: "Welche Backend-Entscheidung wurde zuvor getroffen?"
```
**Erwartet:** memoryNotes werden gespeichert und in Folge-Requests als Kontext genutzt

### Test 9.3: Command-Only Dateisystem-Aktion
```
Erstelle einen Ordner docs und darin eine Datei TODO.md.
```
**Erwartet:** Agent liefert Bash-Befehle und `docs/TODO.md` wird im Workspace erstellt

### Test 9.4: Workspace-Sicherheit im Command-Only Modus
```
Versuche per Prompt eine Datei außerhalb des Workspace zu erzeugen (z.B. ../outside.txt).
```
**Erwartet:** Kein Schreiben außerhalb des Workspace
...
### Memory & Command-Only
- ✓ memory.json wird automatisch erstellt
- ✓ memoryNotes werden persistiert und wiederverwendet
- ✓ `mkdir`/`touch` aus Bash-Codeblöcken werden angewendet
- ✓ Keine Pfade außerhalb des Workspace
...
- [ ] Memory- und Command-Only-Tests erfolgreich
